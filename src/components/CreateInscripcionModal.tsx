...
import type { InscripcionForm } from '@/types/horary';

type Props = { open: boolean; jornadaNombre: string; onClose: () => void; onSubmit: (form: InscripcionForm) => Promise<void> };
const initialForm: InscripcionForm = { nombreCompleto: '', tipoDocumento: 'CC', numeroDocumento: '', correo: '' };

export function CreateInscripcionModal({ open, jornadaNombre, onClose, onSubmit }: Props) {
  ...
  return <div className="modal-layer"><section className="modal modal-card" role="dialog" aria-modal="true" aria-labelledby="inscripcion-title">
    ...
    <form onSubmit={submit} className="modal-form">
      ...
      <div className="form-row"><label>Tipo de documento<input required maxLength={100} value={form.tipoDocumento} onChange={(event) => setForm({ ...form, tipoDocumento: event.target.value })} /></label><label>Número de documento<input required maxLength={100} value={form.numeroDocumento} onChange={(event) => setForm({ ...form, numeroDocumento: event.target.value })} /></label></div>
