...
export function DateRangeField({ from, to, onApply }: Props) {
  ...
  function openPicker() {
    ...
    setMonthYearOpen(false); setOpen(true);
  }
  ...
  return <div className="range-picker" ref={ref}>
    <button type="button" className="range-trigger" onClick={openPicker} aria-expanded={open}><span className="calendar-icon">▣</span><span>{label}</span><i className={`dropdown-chevron ${open ? 'up' : ''}`} /></button>
    {open && <div className="range-popover" role="dialog" aria-label="Selector de rango de fechas">
      ...
      <div className="range-body"><div className="range-calendar"><div className="calendar-header"><button type="button" className="chevron-button previous" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))} aria-label="Mes anterior"><i /></button><button type="button" className="month-year-trigger" onClick={() => setMonthYearOpen(!monthYearOpen)} aria-expanded={monthYearOpen}>{months[cursor.getMonth()]} de {cursor.getFullYear()} <i className={`dropdown-chevron ${monthYearOpen ? 'up' : ''}`} /></button><button type="button" className="chevron-button next" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))} aria-label="Mes siguiente"><i /></button></div>{monthYearOpen && <div className="month-year-picker"><div className="year-picker"><button type="button" className="chevron-button previous" onClick={() => setCursor(new Date(cursor.getFullYear() - 1, cursor.getMonth(), 1))} aria-label="Año anterior"><i /></button><strong>{cursor.getFullYear()}</strong><button type="button" className="chevron-button next" onClick={() => setCursor(new Date(cursor.getFullYear() + 1, cursor.getMonth(), 1))} aria-label="Año siguiente"><i /></button></div><div className="month-grid">{months.map((month, index) => <button type="button" className={index === cursor.getMonth() ? 'chosen' : ''} key={month} onClick={() => { setCursor(new Date(cursor.getFullYear(), index, 1)); setMonthYearOpen(false); }}>{month.slice(0, 3)}</button>)}</div></div>}<div className="calendar-week">{weekDays.map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map((day, index) => { const value = day ? keyOf(new Date(cursor.getFullYear(), cursor.getMonth(), day)) : ''; const inRange = value && draftFrom && draftTo && value >= draftFrom && value <= draftTo; return day ? <button type="button" key={index} className={`${value === draftFrom || value === draftTo ? 'selected-day' : ''} ${inRange ? 'in-range' : ''}`} onClick={() => chooseDay(day)}>{day}</button> : <span key={index} />; })}</div></div><aside className="range-presets"><strong>Periodos rápidos</strong><button type="button" onClick={() => preset(1)}>Hoy</button><button type="button" onClick={() => preset(7)}>Últimos 7 días</button><button type="button" onClick={() => preset(14)}>Últimos 14 días</button><button type="button" onClick={() => preset(30)}>Últimos 30 días</button></aside></div>
